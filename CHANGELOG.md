...
- Errors thrown from `\PayBridgeNP\PayBridge` calls are now typed exceptions (`AuthenticationException`, `AccountException`, `PermissionException`, `InvalidRequestException`, `IdempotencyException`, `RateLimitException`). Branch with `instanceof` instead of inspecting status codes.
